...
int LandsharkJoystick::step( const radl_in_t *in, const radl_in_flags_t *in_flags, radl_out_t *out, radl_out_flags_t *out_flags ) 
{
  ...
  if ( ( get_status() != JOYSTICK_OPEN )  ) {
    radl_turn_on( radl_TIMEOUT, &out_flags->base );
    radl_turn_on( radl_TIMEOUT, &out_flags->over_ride );
    radl_turn_on( radl_TIMEOUT, &out_flags->turret_pan );
    radl_turn_on( radl_TIMEOUT, &out_flags->turret_tilt );
    radl_turn_on( radl_TIMEOUT, &out_flags->moog_pan );
    radl_turn_on( radl_TIMEOUT, &out_flags->moog_tilt );
    radl_turn_on( radl_TIMEOUT, &out_flags->trigger );
  }
  else if ( ( fresh_ != true && zero == true ) ) {
