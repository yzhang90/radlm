...
class Joystick
{
  ...
  public: 
    inline void set_status( joystick_status_t value ) {
      boost::mutex::scoped_lock lock( status_mutex_ );
      status_ = value;
    }

    inline joystick_status_t get_status( ) {
      boost::mutex::scoped_lock lock( status_mutex_ );
      return status_;
    }
        ...
        while ( opened && get_run() ) {
          ...
          if ( select_ret > 0 ) {
            ...
            if ( FD_ISSET(joy_fd, &set)  ) {
              ...
              std::cout << "unknown error!" << std::endl;
              set_status( JOYSTICK_ERROR_FD_ISSET );
            }
          }
          else if ( select_ret == 0 ) {
          }
          else { // if (val < 0 ) 
            std::cout << "[joy] error in select()" << std::endl;
            perror( "[joy] error in select()" );
            usleep( 10000 );
            set_status( JOYSTICK_ERROR_SELECT );
          }
          usleep( 5000 );
        }
