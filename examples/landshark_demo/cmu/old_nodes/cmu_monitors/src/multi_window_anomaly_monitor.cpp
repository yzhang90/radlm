...
void AnomalyMonitor::init(ros::NodeHandle* n,
                          bool use_sample_var){
  bool debug = true;
  if(debug) printf("initializing anomaly monitor\n");
...
//FIX  
//  anomalyPublisher=n->advertise<cmu_sensor_monitor::AnomalyMsg>(
//      "/landshark/ExecutionMonitor/AnomalyMonitor",1);

  double deviation;
  n->param("/cmu_sensor_monitor/max_anomaly", anomaly_thresh,0.9);
  n->param("/cmu_sensor_monitor/obs_variance", variance,0.001);
  n->param("/cmu_sensor_monitor/acceptable_deviation", deviation,0.001);
  acceptableDeviation.push_back(deviation);

  if(debug) printf("Anomaly monitor initialized\n");
}

void AnomalyMonitor::calculateMaxAnomaly() {
  
//  printf("AnomalyMonitor::calculateMaxAnomaly():\n");
...
//  printf("AnomalyMonitor::flushNewData():\n");
...
{
//  printf("\nAnomalyMonitor::calculateAnomaly");
...
//  printf("\nAnomalyMonitor::run():\n");
  ...
  calculateMaxAnomaly();
//FIX
//  cmu_sensor_monitor::AnomalyMsg msg;
