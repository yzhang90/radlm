...
#            Jason Larkin (jason.larkin@spiralgen.com)
# 
# $Id$
#
...
class LandsharkWayPointController:
  ...
  def Initialize(self):
    ...
    odometryTopic = rospy.get_param('odometry_topic', '/landshark/odom_true')
...
#----------------------------------------------------------------------------------    
    print gpsXyzTopic, '\n'
    print odometryTopic, '\n'
    print velocityCommandTopic, '\n'
    print geometry_msgs.msg.PointStamped, '\n'
    print nav_msgs.msg.Odometry, '\n'
    print self.GpsXyzCallback
    print self.OdometryCallback
    print velocityCommandTopic, '\n'
    print self.velocityCommandPublisher, '\n'
    print geometry_msgs.msg.TwistStamped, '\n'
...
if __name__ == '__main__':
  ...
  #read in the file
  if(len(sys.argv) < 2):
    sys.stderr.write("Must pass file name in as command line argument")
    sys.exit()
  f_name = sys.argv[1]
  f = open(f_name)
  for line in f:
    ...
    print (line,s)
    if(len(s)>=2):
      WAY_POINT_LIST.append((float(s[0]), float(s[1])))
