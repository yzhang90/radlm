...
def publisherBaseVelocity(baseVel, angVel):
  if sys.argv[3] == 'deadman_only':
#turn deadman on, so you can move...
    deadmanCommandTopic = rospy.get_param('deadman_command_topic','/landshark_control/deadman')
    deadmanCommandPublisher = rospy.Publisher(deadmanCommandTopic, landshark_msgs.msg.BoolStamped)
