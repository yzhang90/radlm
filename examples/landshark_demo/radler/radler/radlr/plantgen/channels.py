...
def max_stale_steps(pub_period, channel_maxlatency, sub_period, pub_wcet=None):
    ...
    if pub_wcet == None:
        pub_wcet = pub_period
    return ceil((pub_period + pub_wcet + channel_maxlatency)/sub_period)
...
class channel(id_generator):
    def __init__(self, sub, pub, maxlatency, topic, plantinfo):
        """
...
def compute_channels(node, plantinfo):
    ...
    for sub in node['SUBSCRIBES']:
        ...
        c = channel(node, npub, int(sub['MAXLATENCY']._val), t, plantinfo)
