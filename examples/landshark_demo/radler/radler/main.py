...
from radler.radlr.plantgen import channels, kestrel, plantcompute
...
def compile_source(ws_dir=None, no_pervasives=None, pervasives_dir=None,
                   ...
                   plant=None, ROS=None, ROSplant=None, relative_links=None, **_):
    ...
    if ROS:
        def pub_node(t):
            # Resolve publisher nodes
            try:
                return infos.root_namespace.lookup_node(infos.publishers[t._qname])
            except KeyError:
                return None
        rospackage.do_pass(infos.ast, pub_node)

    if plant:
        plant = resolve_plant(plant)
        plantinfo = plantcompute.do_pass(plant)
        channels.do_pass(plantinfo)
        kestrel.do_pass(plantinfo)
        if ROSplant:
            rosplant.do_pass(plantinfo)
...
if __name__ == "__main__":
    ...
    compilep.add_argument('--ROSplant', action='store_true', help='Generate all nodes required by the plant (requires --plant)')
